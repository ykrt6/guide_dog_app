...
def sound(object_dict:list, prediction, dt_now:str, before_second:int):
	...
	if object_dict:
		...
		if (before_second + sound_interval) >= 60:
			flag = True if (((dt_second+60) - before_second) >= sound_interval) else False
		else:
			flag = True if ((dt_second - before_second) >= sound_interval) else False

		if flag:
			if new_object_dict:
				extra = new_object_dict
